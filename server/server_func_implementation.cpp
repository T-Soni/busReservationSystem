...
void handleClient(int clientSocket) {
    ...
    while (read(clientSocket, &code, sizeof(code)) > 0) {
        switch (code) {
            case REGISTRATION_CODE: registerUser(clientSocket); break;
            case LOGIN_CODE: loginUser(clientSocket); break;
            case VIEW_BUSES_CODE: viewBuses(clientSocket); break;
            case BOOK_SEAT_CODE: bookSeat(clientSocket); break;
...
void saveUsersToFile() {
    ...
    for (auto& [uname, user] : users) {
        ...
        for (int seat : user.bookedSeats) fout << " " << seat;
        fout << "\n";
    }
    fout.close();
}
